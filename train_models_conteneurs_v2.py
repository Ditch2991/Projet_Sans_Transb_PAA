...
OUTPUT_MDL      = "models_conteneurs_v2.pkl"
OUTPUT_SER      = "series_conteneurs_v2.pkl"
...
# ─────────────────────────────────────────────────────────────────
# 4. TOTAL = NT + TC2 + Habituel (pas de modèle séparé)
# ─────────────────────────────────────────────────────────────────
print(f"\n[3] Architecture : Total = NT + TC2 constant + Habituel résiduel")
print(f"    → Le modèle NT (WMAPE) pilote le Total")

# Garder y_total pour les clés de répartition terminaux
...
transb_hab_2025 = ann(series['Transb. habituel'], ANNEE_FIN)
print(f"\n[5] TRANSBORDÉ TC2 — constant = {transb_tc2_2025:,} TEU (réalisé {ANNEE_FIN})")
print(f"    TRANSBORDÉ HAB 2025 = {transb_hab_2025:,} TEU")
...
# ─────────────────────────────────────────────────────────────────
# 9b. RATIO TOTAL/NT PAR ANNÉE (clé de projection)
# ─────────────────────────────────────────────────────────────────
ratio_tot_nt = {}
for yr in range(ANNEE_DEBUT, ANNEE_FIN + 1):
    nt_yr  = ann(series['Non transb.'], yr)
    tot_yr = ann_total[yr]
    ratio_tot_nt[yr] = round(tot_yr / nt_yr, 6) if nt_yr > 0 else 1.0

print(f"\n[9b] Ratios Total/NT par année :")
for yr, r in ratio_tot_nt.items():
    print(f"     {yr}: {r:.4f}")
...
# ─────────────────────────────────────────────────────────────────
model_data = {
    # Total = NT + TC2 + Hab (Option A)
    'err_tot'         : None,
    'rmse_tot'        : rmse_nt,   # RMSE du NT (pilote)
    'y_total'         : y_total,
